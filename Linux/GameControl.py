...
while True:
	...
	cs.send("1_0_1_3_-2_-1_1~3_1_1_Sensor On_Sensor Off_S:_-2_-2_0~4_1_DPad:_-1_-1_1~1_0_0_5_-2_-1_1.5~1_2_1_17_-1_-1_1~1_2_1_17_-1_-1_1~0_4_1_BTN1:_-1_-1_1~0_4_2_BTN2:_-1_-1_1~0_4_3_BTN3:_-1_-1_1~0_3_4_BTN4:_-1_-1_1~0_3_5_BTN5:_-1_-1_1~0_3_6_BTN6:_-1_-1_1")
	msg = cs.recv(4096).decode('ascii').strip()
	cs.send("110000")
	msg = cs.recv(4096).decode('ascii').strip()
	while not msg == "":
		try:
			...
			if msg[:5] == "DPad:":
				l = msg.split(":")
				d.emit(u.ABS_X,int(l[1])*50)
				d.emit(u.ABS_Y,int(l[2])*50)
			if msg[:4] == "Acc:" and SO == True:
				l = msg.split(":")
				print int(float(l[1])*10000)
				d.emit(u.ABS_X,int(float(l[1])*10000))
				d.emit(u.ABS_Y,int(float(l[2])*10000))
			if msg[:2] == "S:":
				SO = msg[2:] == "1"
			if msg[:7] == "DIALOG:":
				print msg		
